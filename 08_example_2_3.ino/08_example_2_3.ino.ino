...
int last_raw; // new variable for the exception of the brightness
...
// millis() returns the number of milliseconds since the program started. Will overflow after 50 days.
  ...
  last_raw = dist_raw; // save last measure in case of dist_raw = 0
...
// update last sampling time
  ...
  if (dist_raw >= 200) {
    int led = map(dist_raw, 200, 300, 0, 255);
    analogWrite(PIN_LED, led);
    delay(200);
  }
  else if (dist_raw < 200) {
    dist_raw += 100;
    int led = map(dist_raw, 200, 300, 255, 0);
    analogWrite(PIN_LED, led);
    delay(100);
  }
}
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min) reading = 100.0;
  else if (reading > dist_max) reading  = 300.0;
  return reading;
